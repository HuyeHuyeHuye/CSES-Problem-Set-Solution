...
int fun(int id, int milse){
    if(milse == m) return p26M[n-id];
    if(id == n) return 0;
    
    if(dp[id][milse] != -1) return dp[id][milse];
    
    int ans = 0;
    for(int i = 0; i < 26; i++){
        int j = milse;
        while(j && i != s[j]-'A') j = pi[j-1];
        if(i == s[j]-'A') j++;
        ans += fun(id+1, j);
        ans %= M;
    }
    return dp[id][milse] = ans;
}
